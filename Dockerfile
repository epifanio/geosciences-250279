...
WORKDIR /src
RUN git clone https://github.com/OSGeo/grass /src/grass_build/
...
WORKDIR /tmp
RUN cp -r /src/grass_build/docker/testdata/simple.laz .
# COPY docker/testdata/simple.laz .
WORKDIR /scripts
RUN cp -r /src/grass_build/docker/testdata/test_grass_session.py .
